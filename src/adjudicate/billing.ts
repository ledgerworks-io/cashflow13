 ...
 * controllo: dipende da quanto sono selettivi i criteri del cliente. Nel caso
 * normale (~35% aggiudicabile) il conto vale il 44% del costo dei dati e il
 * tetto non scatta mai. Serve nel caso brutto — quando quasi tutto è
 * aggiudicabile e il listino da solo farebbe costare la revisione più del dato
 * che revisiona.
 *
...
export interface BillingPolicy {
  ...
  /** Tetto, come frazione di quanto il cliente dichiara di aver speso. */
  capFractionOfDeclaredSpend: number;
}
...
export const DEFAULT_POLICY: BillingPolicy = {
  ...
  capFractionOfDeclaredSpend: 0.5,
};
...
export interface Charge {
  ...
  grossUsd: number;
  /** Il tetto, se il cliente ha dichiarato quanto ha speso. */
  capUsd: number | null;
  capApplied: boolean;
...
export function computeCharge(
  ...
  // Promessa 3. Un importo dichiarato assurdo (negativo, NaN, infinito) non
  // apre un buco: semplicemente non c'è tetto.
  const dichiarato = tetto(amountPaidUsd);
  const capUsd = dichiarato === null ? null : usd(dichiarato * policy.capFractionOfDeclaredSpend);
  ...
  return {
    ...
    grossUsd,
    capUsd,
    capApplied: capUsd !== null && capUsd < grossUsd,
    platformCapUsd,
